...
from aiogram.types import (
    ...
    KeyboardButton, Message, ReplyKeyboardMarkup, ReplyKeyboardRemove, BotCommand, WebAppInfo
)
...
WEB_APP_URL = ""  # 🌐 Foydalanishda HTTPS bo'lishi kerak, lokalda esa http://127.0.0.1:8080/ ishlashi mumkin
...
async def init_db():
    ...
    async with pool.acquire() as db:
        ...
        for adm_id in ADMINS:
            await db.execute("INSERT INTO admins(tg_id,name,role) VALUES($1,'Super Admin','admin') ON CONFLICT (tg_id) DO NOTHING", adm_id)

        pay_exists = await db.fetchrow("SELECT id FROM payment_methods LIMIT 1")
        if not pay_exists:
            await db.execute("INSERT INTO payment_methods(type, name, details) VALUES('card', 'Uzcard - Asosiy', '8600123456789012 (Eshmatov T.)')")

        defaults = [("shop_lat", "41.311081"), ("shop_lon", "69.240562"), ("fee_per_km", "5000"), ("is_open", "1")]
        for k, v in defaults:
            await db.execute("INSERT INTO settings(key,value) VALUES($1,$2) ON CONFLICT (key) DO NOTHING", k, v)

        cat_exists = await db.fetchrow("SELECT id FROM categories LIMIT 1")
        if not cat_exists:
            await db.executemany("INSERT INTO categories(name) VALUES($1)", [
                ("Mevalar",), ("Ichimliklar",), ("Non mahsulotlari",), ("Aksessuarlar",)
            ])
...
# ──────────────────────────────────────────
# 🌐 AIOHTTP WEB API SERVER (Sayt uchun)
# ──────────────────────────────────────────
...
async def api_products(request):
    try:
        ...
        if category_id:
            prods = await qall(
                "SELECT p.id, p.category_id, p.name, p.description, p.price, p.stock, p.photo_id, c.name AS category_name "
        ...
        return web.json_response(prods, headers={'Access-Control-Allow-Origin': '*'})
    except Exception:
        return web.json_response([], headers={'Access-Control-Allow-Origin': '*'})

async def api_categories(request):
    cats = await qall("SELECT id, name FROM categories WHERE is_active=1")
    return web.json_response(cats, headers={'Access-Control-Allow-Origin': '*'})

async def api_payment_methods(request):
    pms = await qall("SELECT id, type, name, details FROM payment_methods WHERE is_active=1")
    return web.json_response(pms, headers={'Access-Control-Allow-Origin': '*'})
...
async def api_order(request):
    try:
        ...
        payment_type = str(data.get('payment_type', 'Naqd')).strip() or 'Naqd'
        items = data.get('items', [])
        lat = float(data.get('lat', 0) or 0.0)
        lon = float(data.get('lon', 0) or 0.0)

        if not name or not phone or not landmark:
            raise ValueError('Iltimos, ism, telefon va manzilni to‘liq kiriting.')
        if not isinstance(items, list) or len(items) == 0:
            raise ValueError('Savatingiz bo‘sh. Mahsulot qo‘shing.')
        if lat == 0.0 and lon == 0.0:
            raise ValueError('Iltimos, lokatsiyangizni ruxsat bilan yuboring.')
        ...
        for item in items:
            ...
            if pid <= 0 or quantity <= 0:
                raise ValueError('Noto‘g‘ri mahsulot maʼlumotlari.')
            product = await q1('SELECT name, price, stock FROM products WHERE id=? AND is_active=1', (pid,))
            if not product:
                raise ValueError(f'Mahsulot topilmadi: {pid}')
            if quantity > product['stock']:
                raise ValueError(f"{product['name']} omborda yetarli emas.")
            amount = product['price'] * quantity
            total_amount += amount
            order_items.append({'product_id': pid, 'product_name': product['name'], 'quantity': quantity, 'price': product['price']})

        shop_lat = float((await q1("SELECT value FROM settings WHERE key='shop_lat'"))['value'])
        shop_lon = float((await q1("SELECT value FROM settings WHERE key='shop_lon'"))['value'])
        fee_per_km = int((await q1("SELECT value FROM settings WHERE key='fee_per_km'"))['value'])
        distance = calc_km(shop_lat, shop_lon, lat, lon)
        delivery_fee = int(max(1, math.ceil(distance)) * fee_per_km)

        user = await q1('SELECT tg_id FROM users WHERE phone=?', (phone,))
        user_id = int(user['tg_id']) if user else 0

        order_row = await q1(
            "INSERT INTO orders(user_id, total_amount, payment_method_id, phone, lat, lon, landmark, receipt_id, distance, delivery_fee) VALUES(?,?,?,?,?,?,?,?,?,?) RETURNING id",
            (user_id, total_amount, payment_type, phone, lat, lon, landmark, '', distance, delivery_fee)
        )
        ...
        for item in order_items:
            ...
            await exe('UPDATE products SET stock=GREATEST(0, stock-?) WHERE id=?', (item['quantity'], item['product_id']))

        admin_text = f"🆕 <b>Yangi veb buyurtma #{order_id}</b>\n👤 {name} | 📱 {phone}\n🏠 Mo'ljal: {landmark}\n📏 Masofa: {distance:.1f} km\n💰 Yo'lkira: {fmt(delivery_fee)} so'm\n💳 To'lov: {payment_type}\n\n"
        for item in order_items:
            admin_text += f"▫️ {item['product_name']} x {item['quantity']} = {fmt(item['price'] * item['quantity'])}\n"
        admin_text += f"\n💰 Jami: {fmt(total_amount)} so'm"

        admins = await qall("SELECT tg_id FROM admins WHERE role='admin' AND is_active=1")
        admin_buttons = [[("🚚 Yo'lkira kiritish", f"asetfee_{order_id}" )], [("❌ Rad etish", f"acancel_{order_id}" )]]
        if user_id:
            admin_buttons.append([("💬 Mijozga xabar", f"admmsg_{user_id}")])
        for a in admins:
            try:
                ...
                await bot.send_message(
                    a['tg_id'],
                    admin_text,
                    reply_markup=ik(*admin_buttons)
                )
            except: pass

        return web.json_response({'ok': True, 'order_id': order_id, 'delivery_fee': delivery_fee}, headers={'Access-Control-Allow-Origin': '*'})
    except Exception as e:
        return web.json_response({'ok': False, 'error': str(e)}, status=400, headers={'Access-Control-Allow-Origin': '*'})
...
def ik(*rows): return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=t, callback_data=c) for t, c in row] for row in rows])
def rk(*rows): return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text=t) if isinstance(t, str) else t for t in row] for row in rows], resize_keyboard=True)

CANCEL_KB = rk(["❌ Bekor qilish"])

def web_url_kb():
    url = WEB_APP_URL or "http://127.0.0.1:8080/"
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📱 Web Katalogni ochish", url=url)]])
...
async def build_cart_msg(user_id: int):
    items = await qall("SELECT c.id, c.product_id, c.quantity, p.name, p.price, p.stock FROM cart c JOIN products p ON c.product_id=p.id WHERE c.user_id=?", (user_id,))
    ...
    for i in items:
        ...
        rows.append([("➖", f"cxdec_{i['product_id']}"), (f"{i['name'][:12]}", "noop"), ("➕", f"cxinc_{i['product_id']}")])
...
async def web_app_receive(msg: Message, state: FSMContext):
    try:
        items = json.loads(msg.web_app_data.data)
    except Exception:
        return await msg.answer('❌ Web maʼlumot noto‘g‘ri formatda kelmoqda.')
    if not isinstance(items, list):
        items = [items]

    await exe("DELETE FROM cart WHERE user_id=?", (msg.from_user.id,))
    for i in items:
        pid = int(i.get('id', 0))
        qty = int(i.get('quantity', 1)) if i.get('quantity') else 1
        if pid <= 0 or qty <= 0:
            continue
        await exe(
            "INSERT INTO cart(user_id,product_id,quantity) VALUES(?,?,?) ON CONFLICT(user_id,product_id) DO UPDATE SET quantity=cart.quantity+?",
            (msg.from_user.id, pid, qty, qty)
        )

    txt, kb = await build_cart_msg(msg.from_user.id)
    await msg.answer(f"✅ <b>Web Katalogdan savatga qo'shildi!</b>\n\n{txt}", reply_markup=kb)
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    if not u:
        ...
        await msg.answer(f"👋 Salom, {u['name']}!\nDo'kon holati: {shop_st}", reply_markup=await main_kb(msg.from_user.id))
        if not WEB_APP_URL.startswith('https://'):
            await msg.answer("📱 Web katalogni to‘g‘ridan-to‘g‘ri ochish uchun tugmani bosing:", reply_markup=web_url_kb())
...
async def reg_name(msg: Message, state: FSMContext):
    ...
    await msg.answer("📱 Telefon raqamingizni yuboring:", reply_markup=rk([KeyboardButton(text="📱 Raqam yuborish", request_contact=True)]))
...
async def reg_phone(msg: Message, state: FSMContext):
    ...
    await exe("INSERT INTO users(tg_id,name,phone) VALUES(?,?,?) ON CONFLICT (tg_id) DO NOTHING", (msg.from_user.id, d["name"], ph))
    await state.clear()
...
async def cmd_profile(msg: Message, state: FSMContext):
    ...
    if u and u != "blocked":
        await msg.answer(f"👤 <b>Profilingiz</b>\n\n🆔 Ism: {u['name']}\n📱 Telefon: {u['phone']}", reply_markup=ik([("✏️ Ismni o'zgartirish", "edit_name")]))
...
async def cmd_catalog(msg: Message, state: FSMContext):
    ...
    await msg.answer("📂 <b>Bo'limni tanlang:</b>", reply_markup=ik(*[[(f"📦 {c['name']}", f"cat_{c['id']}")] for c in cats]))
...
async def show_cat(call: CallbackQuery):
    ...
    kb = ik(*[[(f"🛒 {p['name']} - {fmt(p['price'])} so'm", f"prod_{p['id']}")] for p in prods], [("🔙 Orqaga", "back_cats")])
    try: await call.message.edit_text("🛍 <b>Mahsulotlar:</b>", reply_markup=kb)
    except: pass
    await call.answer()
...
async def back_cats(call: CallbackQuery):
    cats = await qall("SELECT * FROM categories WHERE is_active=1")
    try: await call.message.edit_text("📂 <b>Bo'limni tanlang:</b>", reply_markup=ik(*[[(f"📦 {c['name']}", f"cat_{c['id']}")] for c in cats]))
    except: pass
    await call.answer()
...
async def show_prod(call: CallbackQuery):
    ...
    if p["photo_id"]: await bot.send_photo(call.from_user.id, p["photo_id"], caption=txt, reply_markup=ik(*btns))
    else: await bot.send_message(call.from_user.id, txt, reply_markup=ik(*btns))
    await call.answer()
...
async def cart_inc(call: CallbackQuery):
    ...
    ci = await q1("SELECT quantity FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    if p and ci and ci["quantity"] >= p["stock"]: return await call.answer(f"Omborda faqat {p['stock']} ta bor!", show_alert=True)
    await exe("INSERT INTO cart(user_id,product_id,quantity) VALUES(?,?,1) ON CONFLICT(user_id,product_id) DO UPDATE SET quantity=cart.quantity+1", (call.from_user.id, pid))
...
async def cart_dec(call: CallbackQuery):
    pid = int(call.data[5:])
    ci = await q1("SELECT quantity FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    if ci and ci["quantity"] == 1: await exe("DELETE FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    elif ci: await exe("UPDATE cart SET quantity=quantity-1 WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    await show_prod(call)
...
@router.message(F.text == "📱 Web Katalog")
async def cmd_web_catalog(msg: Message, state: FSMContext):
    url = WEB_APP_URL or "http://127.0.0.1:8080/"
    await msg.answer(f"🌐 Web katalogni ochish: <a href=\"{url}\">Bu yerni bosing</a>", parse_mode=ParseMode.HTML)

@router.message(Shop.search)
async def do_search(msg: Message, state: FSMContext):
    res = await qall("SELECT * FROM products WHERE is_active=1 AND name ILIKE ? LIMIT 10", (f"%{msg.text.strip()}%",))
    if not res: return await msg.answer("❌ Topilmadi. Boshqa so'z yozing:")
    await msg.answer("🔍 Natijalar:", reply_markup=ik(*[[(f"🛒 {p['name']} - {fmt(p['price'])}", f"prod_{p['id']}")] for p in res]))
    await state.clear()
...
async def cmd_cart(msg: Message, state: FSMContext):
    if await check_user(msg, state) == "blocked": return
    txt, kb = await build_cart_msg(msg.from_user.id)
    if kb: await msg.answer(txt, reply_markup=kb)
...
async def cxdec(call: CallbackQuery):
    ...
    ci = await q1("SELECT quantity FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    if ci:
        if ci["quantity"] <= 1: await exe("DELETE FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
        else: await exe("UPDATE cart SET quantity=quantity-1 WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    txt, kb = await build_cart_msg(call.from_user.id)
    try:
        if kb: await call.message.edit_text(txt, reply_markup=kb)
        else: await call.message.edit_text(txt)
    except: pass
    await call.answer()
...
async def checkout_start(call: CallbackQuery, state: FSMContext):
    ...
    items = await qall("SELECT c.quantity, p.id, p.name, p.stock FROM cart c JOIN products p ON c.product_id=p.id WHERE c.user_id=?", (uid,))
    ...
    btn = [KeyboardButton(text="📍 Xaritadan lokatsiya yuborish", request_location=True), KeyboardButton(text="❌ Bekor qilish")]
...
async def checkout_landmark(msg: Message, state: FSMContext):
    ...
    for pm in pms:
        if pm['type'] == 'card': btns.append([(f"💳 {pm['name']}", f"pay_{pm['id']}")])
        elif pm['type'] in ['click', 'payme']: btns.append([(f"📱 {pm['name']}", f"pay_{pm['id']}")])
...
async def checkout_payment(call: CallbackQuery, state: FSMContext):
    uid = call.from_user.id; pay_data = call.data[4:]
    d = await state.get_data()
    items = await qall("SELECT c.*, p.price, p.name FROM cart c JOIN products p ON c.product_id=p.id WHERE c.user_id=?", (uid,))
...
async def checkout_receipt(msg: Message, state: FSMContext):
    ...
    await finalize_order(msg, state, msg.from_user.id, pm['name'], d['tot'], d['lat'], d['lon'], d['landmark'], d['items'], msg.photo[-1].file_id)

@router.callback_query(F.data == "force_checkout")
async def force_checkout(call: CallbackQuery, state: FSMContext):
    d = await state.get_data()
    pm = await q1("SELECT name FROM payment_methods WHERE id=?", (int(d['pm_id']),))
    await finalize_order(call.message, state, call.from_user.id, f"{pm['name']} (Elektron)", d['tot'], d['lat'], d['lon'], d['landmark'], d['items'], "")
    try: await call.message.delete()
    except: pass
    await call.answer()
...
async def finalize_order(msg, state, uid, pay_name, tot, lat, lon, landmark, items, receipt_id):
    ...
    items_txt = ""
    for i in items:
        await exe("INSERT INTO order_items(order_id,product_id,product_name,quantity,price) VALUES(?,?,?,?,?)", (oid, i['product_id'], i['name'], i['quantity'], i['price']))
        await exe("UPDATE products SET stock=GREATEST(0, stock-?) WHERE id=?", (i["quantity"], i["product_id"]))
        items_txt += f"▫️ {i['name']} x {i['quantity']} = {fmt(i['price'] * i['quantity'])}\n"

    await exe("DELETE FROM cart WHERE user_id=?", (uid,))
    await state.clear()
    await msg.answer(f"🎉 <b>Buyurtma #{oid} qabul qilindi!</b>\nTez orada admin yo'lkirani tasdiqlaydi.", reply_markup=await main_kb(uid))
...
async def adm_direct_msg_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("📝 Mijozga xabaringizni yozing:\n<i>(Masalan: Lokatsiyani qaytadan tashlang)</i>", reply_markup=CANCEL_KB)
...
async def adm_direct_msg_send(msg: Message, state: FSMContext):
    ...
    if target_uid:
        try:
            await bot.send_message(target_uid, f"👨‍💻 <b>Do'kon ma'muriyatidan xabar:</b>\n\n{msg.text}")
...
async def my_orders(msg: Message, state: FSMContext):
    ...
    for o in ords:
        ...
        btns = []
        if o["status"] == "new": btns.append([("❌ Bekor qilish", f"ucancel_{o['id']}")])
        elif o["status"] == "delivered":
            if not await q1("SELECT id FROM reviews WHERE order_id=?", (o['id'],)): btns.append([("⭐ Baho berish", f"ureview_{o['id']}")])
        if btns: await msg.answer(txt, reply_markup=ik(*btns))
        else: await msg.answer(txt)
...
async def urev_star(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("Fikringizni yozib yuboring (yoki ⏭ O'tkazish):", reply_markup=ik([("⏭ O'tkazish", "rv_skip")]))
    await state.set_state(ReviewSt.comment)
    await call.answer()

@router.message(ReviewSt.comment)
async def urev_comment(msg: Message, state: FSMContext):
    d = await state.get_data(); oid = d['rev_oid']; rating = d['rev_star']; comment = msg.text
...
async def urev_skip(call: CallbackQuery, state: FSMContext):
    d = await state.get_data(); oid = d['rev_oid']; rating = d['rev_star']
...
async def courier_my_orders(msg: Message):
    ords = await qall("SELECT * FROM orders WHERE courier_id=? AND status IN ('assigned', 'on_way')", (msg.from_user.id,))
    ...
    for o in ords:
        ...
        btns = []
        if o['status'] == 'assigned': btns.append([("🛍 Do'kondan oldim", f"cpickup_{o['id']}")])
        elif o['status'] == 'on_way': btns.append([("✅ Mijozga topshirdim", f"cdeliv_{o['id']}")])
...
async def admin_settings(msg: Message):
    ...
    await msg.answer(f"⚙️ <b>Do'kon sozlamalari:</b>\n\nHolat: {st_text}", reply_markup=ik([("📍 Do'kon lokatsiyasini yangilash", "set_shop_loc")], [("🟢/🔴 Ochiq-Yopiq qilish", "toggle_shop")]))
...
async def set_shop_loc_start(call: CallbackQuery, state: FSMContext):
    btn = [KeyboardButton(text="📍 Xaritadan lokatsiya tashlash", request_location=True), KeyboardButton(text="❌ Bekor qilish")]
...
async def admin_payments(msg: Message):
    ...
    for p in pms:
        ...
        rows.append([(f"{st} {p['name']} ({p['type']})", f"epay_{p['id']}")])
    rows.append([("➕ Yangi usul qo'shish", "add_pay")])
    await msg.answer("💳 <b>To'lov usullari:</b>", reply_markup=ik(*rows))
...
async def add_pay_type(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Turini tanlang:", reply_markup=ik([("💳 Karta", "ptype_card")], [("📱 Click", "ptype_click"), ("📱 Payme", "ptype_payme")]))
    await call.answer()
...
async def save_pay(msg: Message, state: FSMContext):
    d = await state.get_data()
    await exe("INSERT INTO payment_methods(type, name, details) VALUES(?,?,?)", (d['ptype'], d['pname'], msg.text.strip()))
    await state.clear()
...
async def admin_cat_detail(call: CallbackQuery):
    ...
    await call.message.edit_text(f"📦 <b>{c['name']}</b>", reply_markup=ik([("✏️ Nomni o'zgartirish", f"acatedit_{cid}")], [("🗑 O'chirish", f"acatdel_{cid}")]))
    await call.answer()
...
async def adm_prods(msg: Message):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='admin'", (msg.from_user.id,)): return
    prods = await qall("SELECT p.*, c.name as cname FROM products p LEFT JOIN categories c ON p.category_id=c.id LIMIT 40")
...
async def add_prod_cb(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Qaysi bo'limga qo'shamiz?", reply_markup=ik(*[[(c["name"], f"selc_{c['id']}")] for c in cats]))
...
async def pedit_menu(call: CallbackQuery):
    pid = int(call.data[6:])
    await call.message.edit_text("Nimani o'zgartiramiz?", reply_markup=ik(
...
async def save_pedit(msg: Message, state: FSMContext):
    d = await state.get_data()
    pid = d.get("edit_pid"); field = d.get("edit_field")
...
async def adm_orders(msg: Message):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='admin'", (msg.from_user.id,)): return
    ords = await qall("SELECT o.*, u.name as customer FROM orders o LEFT JOIN users u ON u.tg_id=o.user_id ORDER BY o.id DESC LIMIT 20")
    if not ords:
        return await msg.answer("📦 Hozircha buyurtmalar yo'q.")
    for o in ords:
        st = STATUSES.get(o['status'], o['status'])
        txt = f"📦 #{o['id']} | Holati: {st}\n👤 {o.get('customer','Anonim')}\n🏠 {o['landmark']}\n💰 Jami: {fmt(o['total_amount'] + o['delivery_fee'])} so'm\n📏 Masofa: {o['distance']:.1f} km"
        btns = []
        if o['status'] == 'new': btns.append([("🚚 Yo'lkira kiritish", f"asetfee_{o['id']}")])
        if o['status'] == 'pending_courier': btns.append([("📣 Kuryerga e'lon qilish", f"doconf_{o['id']}_{int(o['delivery_fee'] or 0)}")])
        if o['status'] in ['new', 'pending_courier', 'assigned']:
            btns.append([("❌ Bekor qilish", f"acancel_{o['id']}")])
        await msg.answer(txt, reply_markup=ik(*btns) if btns else None)
...
# ──────────────────────────────────────────
# 🚀 ENGINE RUNNER (Dvigatel - HAM BOT, HAM API)
# ──────────────────────────────────────────
async def main():
    await init_db()
